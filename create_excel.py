...
ws_veri.sheet_properties.tabColor = "0070C0"  # Blue

# Headers for Veri sheet
...
for row_num in range(2, len(raw_data) + 2):
    ...
    for col_num in range(1, len(headers_veri) + 1):
        ...
        elif col_num in [3, 9, 10, 11]:  # Integer counts and totals
            cell.number_format = '#,##0'
            cell.alignment = Alignment(horizontal="right")
        elif col_num in [4, 5, 6, 7]:  # Decimals (amounts and counts)
            cell.number_format = '#,##0.00'
            cell.alignment = Alignment(horizontal="right")
        elif col_num == 8:  # Discount percentage
...
ws_ozet.sheet_properties.tabColor = "00B050"  # Green
...
# Build formulas for each month
current_row = 2
for month in months:
    # Find row indices for this month in Veri sheet (0-indexed, so add 2 to get Excel row)
    month_rows = [idx for idx, row in enumerate(raw_data) if row[0] == month]

    # Ay
    ws_ozet.cell(row=current_row, column=1).value = month

    # Get Excel row numbers for this month's data in Veri sheet
    veri_row_3al = month_rows[0] + 2  # 3Al2Ode
    veri_row_diger = month_rows[1] + 2  # DigerKampanya
    veri_row_kamp = month_rows[2] + 2  # Kampanyasiz
...
for col_num in range(2, 26):
    ...
    # Average for sepet and ürün columns, sum for fiş and ciro
    if col_num in [2, 3, 4, 5]:  # Fiş counts
        cell.value = f"=SUM({col_letter}2:{col_letter}{total_row-1})"
    elif col_num in [15, 16, 25]:  # Percentages - average
        cell.value = f"=AVERAGE({col_letter}2:{col_letter}{total_row-1})"
    elif col_num in [20, 24]:  # Total ciros
        cell.value = f"=SUM({col_letter}2:{col_letter}{total_row-1})"
    else:  # Sepet amounts and other metrics
        cell.value = f"=AVERAGE({col_letter}2:{col_letter}{total_row-1})"
...
for row_num in range(2, total_row + 1):
    ...
    for col_num in range(1, 26):
        ...
        if col_num == 1:  # Ay
            cell.alignment = Alignment(horizontal="center")
        elif col_num in [2, 3, 4, 5]:  # Fiş (counts)
            cell.number_format = '#,##0'
            cell.alignment = Alignment(horizontal="right")
        elif col_num in [6, 7, 8, 9, 10, 11]:  # Sepet amounts
            cell.number_format = '#,##0.00'
            cell.alignment = Alignment(horizontal="right")
        elif col_num in [12, 13, 14]:  # Ürün adedi
            cell.number_format = '#,##0.0'
            cell.alignment = Alignment(horizontal="right")
        elif col_num in [15, 16, 25]:  # Percentages
            cell.number_format = '0.0'
            cell.alignment = Alignment(horizontal="right")
        elif col_num in [17, 18, 19, 20, 21, 22, 23, 24]:  # Ciro
            cell.number_format = '#,##0'
            cell.alignment = Alignment(horizontal="right")

# Set column widths for Özet
...
ws_dash.sheet_properties.tabColor = "ED7D31"  # Orange
...
# Placeholder for charts (will be added after)
ws_dash["A12"].value = "Trendler ve Analizler (Tablolardan oluşturulacak)"
