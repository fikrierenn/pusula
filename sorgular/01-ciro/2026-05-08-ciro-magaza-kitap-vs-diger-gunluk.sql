-- =====================================================================
-- CIRO_MAGAZA_KITAP_VS_DIGER_GUNLUK_KARSILASTIRMA.sql
-- Amaç: Mağaza × Grup günlük çapraz rapor — Bugün vs Dün
--        Grup = KİTAP (Akademi+Çocuk Kitabı+Hazırlık Kitapları+Kitap)
--             | DİĞER (kalan tüm kategoriler)
--        Her mağaza için Net Ad / Dün Ad / Ad Δ% / Net TL / Dün TL / Δ TL / Δ%
--        + Toplam blok + Pay % + Dün Pay % + Ort. Birim Fiyat + Birim Δ%
--
-- Temel: 6_CIRO_MAGAZA_KATEGORI_GUNLUK_KARSILASTIRMA.sql
-- Değişim: Kategori3 yerine KitapDigerGrup
...
-- SIRALAMA: KİTAP en üstte, DİĞER, sonra GENEL TOPLAM
-- =====================================================================
...
;WITH HareketKaynak AS (
    SELECT
        ...
        -- Kitap grubu mu, diğer mi?
        CASE
            WHEN UrunBilgi.Kategori3 IN (N'Akademi', N'Çocuk Kitabı', N'Hazırlık Kitapları', N'Kitap')
                THEN N'KİTAP'
            ELSE N'DİĞER'
        END AS KitapDigerGrup,
        CASE
...
MagazaPivot AS (
    ...
    FROM HareketKaynak
    GROUP BY KitapDigerGrup
),
DetayVeToplam AS (
    -- KİTAP en üstte (1), DİĞER ortada (2), GENEL TOPLAM en altta (3)
    SELECT
        CASE WHEN KitapDigerGrup = N'KİTAP' THEN 1 ELSE 2 END AS SiralamaGrubu,
        KitapDigerGrup,
...
SELECT
    ...
    -- ============== Pay yüzdeleri (kitap/diğer dağılım) ==============
    CAST(100.0 *
         ( (Detay.FsmSatisTutarB    - Detay.FsmIadeTutarB)
         + (Detay.OzluceSatisTutarB - Detay.OzluceIadeTutarB)
         + (Detay.IstYoluSatisTutarB- Detay.IstYoluIadeTutarB) )
