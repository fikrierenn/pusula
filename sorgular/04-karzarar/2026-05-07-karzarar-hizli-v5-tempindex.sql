/*================================================================
  Kâr/Zarar v5 (Hızlandırılmış+Temp Index) — Tüm Ürün, 4 Mağaza
  ----------------------------------------------------------------
  Yazan         : Claude (Fikri'nin prod sorgusu + agresif refactor)
  Tarih         : 07.05.2026

  v4 → v5 değişimi:
    v4: Bazı ara adımlar CTE
    v5: HER ara adım temp table + index — optimizer her adımda
        gerçek istatistik üzerinden plan kurar.

  ADIMLAR (her biri atomik, ölçümlü):
    1. #SATILAN_URUN (PK stkId)
       — irs+irsAyr distinct ürün
    2. #FAT_RAW (NCI: stkId,eTarih)
       — son @MALIYET_LOOKBACK_AY ay alış faturası ham
    3. #ODAK_RAW (NCI: stkId,Tarih)
       — BKMDATA..ODAK_FATURA cross-DB ham
    4. #MALIYET_AVG (PK stkId)
       — #FAT_RAW + #ODAK_RAW UNION → TOP 5 → ortalama
    5. #ALISSART (PK stkId)
       — fytOzl son şart × 5 kademeli iskonto
    6. #SATIS_RAW (NCI: stkId,mekanId,tarih)
       — irs+irsAyr+fat+fatAyr ham satış satırları
    7. #MALIYET (NCI: tarih,stkId,mekanId)
       — final kâr/zarar tablosu
    8. Result-set'ler
  ...
  KULLANIM:
    ...
    @MALIYETTIP: 0 = 5-fat avg, >0 = ALISSART
    Süre: SSMS'te 7 günlük aralık < 10s hedef
================================================================*/

USE DerinSISBkm;
SET NOCOUNT ON;
SET STATISTICS TIME OFF;  -- istersen ON yap, her adımın süresini görürsün
...
-- ================================================================
-- ADIM 1: Satış aralığında SATILAN ürünleri (1 PK lookup için)
-- ================================================================
...
-- ================================================================
-- ADIM 2: Satılan ürünlerin son fatura ham kayıtları (DerinSIS)
-- ================================================================
...
INSERT INTO #FAT_RAW(stkId, eTarih, eFirma, ehAdetN, ehTutarN)
SELECT s.stkId, b.eTarih, b.eFirma, b.ehAdetN, b.ehTutarN
FROM #SATILAN_URUN s
CROSS APPLY (
    SELECT TOP 5 fa.ehAdetN, fa.ehTutarN, f.eTarih, f.eFirma
...
-- ================================================================
-- ADIM 3: ODAK_FATURA ham (cross-DB, sadece eksikse fallback)
-- ================================================================
...
-- ================================================================
-- ADIM 4: 5-fat avg maliyet (DerinSIS + ODAK UNION → ortalama)
-- ================================================================
...
-- ================================================================
-- ADIM 5: ALISSART (fytOzl + 5 kademeli iskonto)
-- ================================================================
...
-- ================================================================
-- ADIM 6: SATIS ham — irs+irsAyr+fat+fatAyr birleşim
-- ================================================================
...
CREATE CLUSTERED INDEX CIX_SATIS_RAW_tarih ON #SATIS_RAW (tarih, mekanId, stkId);


-- ================================================================
-- ADIM 7: #MALIYET final — SATIS_RAW × MALIYET_AVG × Aktarim × ALISSART
-- ================================================================
...
SELECT
    s.tarih, s.satisTip, s.mekanId, s.stkId, s.adet, s.tutar,
...
CREATE CLUSTERED INDEX CIX_MALIYET_tarih ON #MALIYET (tarih, mekanId, stkId);
CREATE NONCLUSTERED INDEX IX_MALIYET_stk ON #MALIYET (stkId);
...
BEGIN
    SELECT
        ...
        CAST(SUM(IIF(@MALIYETTIP = 0, M.maliyet, M.adet * AS_.NetAlisFiyat))
              AS DECIMAL(18,2))                                                  AS Maliyet,
        CAST(SUM(M.tutar) - SUM(IIF(@MALIYETTIP = 0, M.maliyet, M.adet * AS_.NetAlisFiyat))
              AS DECIMAL(18,2))                                                  AS Marj_TL,
        CAST(
