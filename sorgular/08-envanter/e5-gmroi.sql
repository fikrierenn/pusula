...
;WITH OrtEnv AS (
    ...
    FROM (SELECT KTGR3,
          ...
          WHERE CAST(Tarih AS date)=@AyBas AND [Maliyet Tipi]='Ort.Maliyet' GROUP BY KTGR3) b
    JOIN (SELECT KTGR3,
            SUM(ISNULL([FSM Stok Maliyet],0)+ISNULL([Özlüce Stok Maliyet],0)+ISNULL([İst.Yolu Stok Maliyet],0)) AS Maliyet
          FROM DerinSISBkm.bkm.ENVANTER_RAPORU WITH(NOLOCK)
          WHERE CAST(Tarih AS date)=@AySon AND [Maliyet Tipi]='Ort.Maliyet' GROUP BY KTGR3) e
      ON e.KTGR3 = b.KTGR3
)
SELECT
    m.KTGR3                                                              AS Kategori,
