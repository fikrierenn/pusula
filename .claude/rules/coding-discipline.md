...
- **Lucide `<i data-lucide>` KOŞULLU render edilmez — circuit çöker (29.08 dersi).** `lucide.createIcons()` `<i>` düğümünü `<svg>` ile DEĞİŞTİRİR; Blazor kendi diff'inde o `<i>`'yi aramaya gidince bulamaz → `TypeError: Cannot read properties of null (reading 'removeChild')` + "An unhandled error has occurred" (circuit terminate). Tetikleyen desen: bir buton içinde `@if (yukleniyor) { <span class="loading"> } else { <i data-lucide=...> }`. **Doğrusu:** ikonu ve spinner'ı SABİT tut, görünürlüğü CSS ile değiştir (`class="loading ... @(yukleniyor ? "" : "hidden")"`). Kardeş düğüm swap'i tehlikeli; tüm alt-ağacın birden kaldırılması (ör. filtreyle kartın komple gitmesi) sorun değil.
...
- **Rapor scripti Türkçe varchar okurken pymssql DEĞİL pyodbc (24.06 dersi).** DerinSIS `varchar` kolonları CP1254 (Turkish_CI_AS) collation; pymssql bunları yanlış decode eder (ı→ý, ş→þ, ğ→ð) → Excel/CSV çıktısında sessiz Türkçe bozulması. **pyodbc + ODBC Driver 18** doğru Unicode (`Arı`=U+0131) verir. Yeni okuma scriptlerinde (Excel/rapor üretimi) pyodbc kullan; pymssql sadece Türkçe-içermeyen/legacy yerde. Host/port `.env`'den + whitelist guard (`re.fullmatch(r"[A-Za-z0-9._\\-]+", host)`).
