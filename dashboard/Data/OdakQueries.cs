    ...
    public async Task<OdakSayfa<OdakUrun>> GetUrunlerAsync(string kategori, int offset, int take)
    {
        ...
        var sql = $"""
            ...
            SELECT
                ...
                ISNULL(mgz.Fsm,0) AS StokFsm, ISNULL(mgz.Ozl,0) AS StokOzl, ISNULL(mgz.Ist,0) AS StokIst,
                ISNULL(mgz.Mrkz,0) AS StokMrkz, ISNULL(o.StokMiktar,0) AS StokOdak,
                ISNULL(mgz.Fsm,0)+ISNULL(mgz.Ozl,0)+ISNULL(mgz.Ist,0)+ISNULL(mgz.Mrkz,0)+ISNULL(o.StokMiktar,0) AS StokToplam,
                ISNULL(sat.sFsm,0) AS SatisFsm, ISNULL(sat.sOzl,0) AS SatisOzl, ISNULL(sat.sIst,0) AS SatisIst,
                CAST(ISNULL(ecom.Qty,0) AS int) AS EcomSiparis,
                ISNULL(sat.sFsm,0)+ISNULL(sat.sOzl,0)+ISNULL(sat.sIst,0)+CAST(ISNULL(ecom.Qty,0) AS int) AS SatisToplam,
