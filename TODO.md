...
- [~] **B-122 Kontrol paneli SP perf** — 24.06, 3 katman: (1) sargable date-range join (YEAR/MONTH→aralık, CAR `IX_car_cTarih`/FAT `IX_fat_7` seek). (2) MHS `fisbSirketID=yıl−2020` → mevcut `mhsFisBaslik_FisListe` index seek (1494→66ms). (3) **`@Donemler` table-variable → `#Donemler` TEMP TABLO + `OPTION(RECOMPILE)`** — table-var 1-satır tahmin ediyordu → car 23.7M full-scan → **240s TIMEOUT** (component'ler hızlı ama SP timeout = klasik table-var kötü-plan). Temp+RECOMPILE → gerçek kardinalite/değer → seek. **YENİ INDEX GEREKMEZ.** Source `sorgular/2026-06-22-muhasebe-kontrol-v2.sql`. **KALAN: Fikri redeploy (3-katman tek dosyada, son sürüm).** + ilk-yük tek-dönem (69e1d36).
