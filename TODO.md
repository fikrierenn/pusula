...
- [~] **B-74 ⚡ PERF progressive-load** — Envanter ✅ 15.06. **Eticaret ✅ 16.06 commit b792f4e** (10 sorgu linked ODAKJOKER yerine .70 DİREKT + LOGOGRUP kategori 4.1s→0.6s — linked latency tamamen kalktı). **Kalan:** Sadakat RfmGecis ağır. Stockout CROSS APPLY (GetInventory) tek GROUP BY pre-agg. **(YENİ 15.06)**
- [ ] **B-75 ⚡ GENEL BAKIŞ REDESIGN (Tier-3)** — kararlar onaylı (AskUserQuestion 16.06): (1) TOPLAM+Mağaza Toplam+E-tic+mağazalar **hepsi `<a href>` sayfa-nav** (modal yok); (2) yeni **`/toplam`** sayfa (3 mağaza+online birleşik kategori, `_modalTot` içeriği); (3) yeni **`/magazalar`** karşılaştırma sayfası (Mağaza Toplam kartı buraya); (4) **/tahmin zenginleştir** (YoY taban+MoM ivme+kalan gün hesap adımları+mağaza/kategori tablo) + Home tahmin kartı ÖZET. **Yöntem:** Explore→harita, planner(opus)→`plans/NN-genel-bakis-redesign.md`, dashboard-icerik kalıbı, context7 API. plan ONAYLANMADAN kod yok. **(YENİ 16.06)**
- [ ] **B-76 sema kaydı** — JOKER direkt köprü (.70 OpenJokerAsync) + `J_ITEMS.DERINSIS_LOGOGRUP`=DerinSIS Kategori3 grain + spc fan-out gotcha (SalesProductCampaigns N satır/ürün → sp.TotalPrice fan-out şişer) → `sema-ogren`. **(YENİ 16.06)**
