...
- [ ] **B-113** UI checklist temizlik (asistan-ui §5.5 taraması 17.06, 35 ihlal — YÜKSEK yok): (a) **Emoji ikon→Lucide SVG** 15 adet (Asistan ağırlıklı: Kaydet/Ata/Düzelt/İptal butonları, Home/Magazalar/Sadakat/Operasyon link-badge, Modal ✕, MainLayout) · (b) **Hardcode hex→DaisyUI** 11 adet ApexChart C# option (AppMultiLine/Stacked/Bar/Area + Magazalar Colors + App theme-color) — charts.js'teki CSS-var okuma kalıbını C# option'a taşı (JS interop) · (c) **focus-visible** 7 input (Asistan/Gorevler/StokHareket/Home). ✅ Türkçe ihlalleri (Error/NotFound/ReconnectModal) 17.06 düzeltildi.
