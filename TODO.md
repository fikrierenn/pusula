...
- [ ] **B-45 ⭐ YAPILACAK (karar 17.06: olacak)** [TIER 3 plan-first] **BKM-Asistan** — minimal AI asistan: Telegram bot + Claude API (tool-use loop) + mevcut sema/Dapper/gm-rapor/MCP araçları. CFO telefondan "dün kargo/ciro ne oldu" → Claude SQL/rapor aracıyla Türkçe cevap. Stack C# (.NET, Telegram.Bot + Anthropic SDK), dashboard ile tutarlı (asistan/ POC mantığı zaten var: prompt/SQLite/parse). **Harness pattern referansı: learn-claude-code (shareAI-lab)** — tool-use loop + dispatch-map (yeni araç=handler register) + permission (s03) + subagent izolasyon (s06) + bağlam-sıkıştırma (s08). Çekirdek = loop+tool+permission; sandbox/multi-kanal GEREKSİZ. Kurmadan önce: güvenlik (tek-kullanıcı auth — B-84 cookie modeliyle hizalı), API maliyet, mimari plan (Tier-3). asistan/BkmAsistan.csproj iskelet mevcut.
