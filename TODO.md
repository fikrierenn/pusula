...
- [~] **B-127 FSM Trafik & Kasiyer + PDKS saatlik personel** — 29-30.06 sayfa (commit 0448dae/bd7942e/a604b53/6f42597: kapı×POS heatmap+KPI+personel tablosu). **01.07 PDKS eklendi (UNCOMMITTED):** `GetPdksPersonelAsync` GecoTime `OPENQUERY([PDKS])` TTagZei×TPerTab, saatlik headcount 3-kural (span MIN-MAX / çıkış-dakika dahil / opdays payda — sema `pdks_saatlik_personel`). 4-bant heatmap → tek "Personel–Trafik Uyumu" karne haritası (renk=mismatch ±0.15, opaklık=yoğunluk) + veri-tablosu modal grid. SQL arşiv `sorgular/2026-06-30-pdks-saatlik-personel.sql` (üretim + 8-sorgu analiz izi). **KALAN:** commit + tarayıcı görsel teyit + FSM-only mu çok-mağaza mı kararı.
