...
- [ ] **HK-0 🥇 İLK İŞ: tüm açık TODO tara + iş listesi + TODO-yönetim skill** (kullanıcı direktifi 17.06) — (1) TODO.md TÜM açık `[ ]` maddeleri tara (todo-verification: file:line ile canlı doğrula, stale [x]-değil-aslında-açık + atlanmış olanları yakala), (2) öncelikli **iş listesi** çıkar (efor/değer/bağımlılık). (3) **TODO-yönetimi için skill GEREKİYOR mu** değerlendir: `plan-tracker` (TodoWrite↔TODO sync) + `consolidate-sema` (stale budama) VAR — bunlar "tüm açık tara + öncelikli iş listesi üret" işini KAPSIYOR mu? Boşluk varsa → **`yetenek-uret`** meta-skill ile `todo-tara` skill'i forge et (footprint-ladder: önce mevcut skill'i genişlet, yetmezse yeni). Kapsıyorsa skill açma.
