...
- ✅ `/gm-rapor` skill + `docs/rapor-katalogu.md` iş haritası + deep-research (perakende KPI best-practice, 16 doğrulanmış iddia).
...
- [ ] **B-22 [TIER 3 plan-first]** FSM kapı sayıcı entegrasyonu — `plans/02-trafik-analiz.md` yazılacak. Scope: (1) sayıcı veri kaynağı tespiti — API/CSV/DB hangi format, (2) SQL Server tabloya kayıt mekanizması, (3) Özlüce + İst.Yolu'na yayma, (4) saat bazlı kırılım, (5) Pazartesi briefing'e "Dönüşüm Oranı" + "Müşteri/giren" KPI kolonları ekleme. **İlk bulgu (28.04 hızlı analiz):** FSM dönüşüm %51,2 (yüksek, sağlıklı), 23 Nis Çocuk Bayramı 2.658 giriş (haftaiçi normalin %50 üstü, brief hipotezi doğrulandı), FSM düşük sepet sorunu trafik/dönüşüm değil **ürün karışım/demografik** — kategori analizi gerekli (B-08 ile bağlantılı). 29-30 Nis verisi şüpheli (future timestamps). **(YENİ)**
...
- [ ] **B-26** ⚡ **E5 GMROI SSMS doğrulaması** — tek doğrulanmamış GM parçası. karzarar v7 (@AyBas/@AySon) çalıştır → ÇIKTI3 Marj_TL'yi KTGR3'e topla → `sorgular/08-envanter/e5-gmroi.sql` `#MARJ`'a doldur → çalıştır. Pay (marj) tarafı MCP'de hiç koşmadı. **(YENİ)**
...
- [ ] **B-30** Haftalık P2-P8 verified-wire (kategori trend, kâr/zarar, kampanya, kasiyer, PDKS) — `docs/rapor-katalogu.md` § B mevcut sorgulara tarih daralt + canlı doğrula. **(YENİ)**
