...
- [~] **B-117 Muhasebe denetim katmanı (GL forensic)** — **22.06 başladı**. Keşif: `DerinSISBkm.mhs` (yevmiye 39,5M, mizan/hesap planı), `dbo.car/carCek/frm/fat` (cari/çek/vade — canlı bugüne kadar), Fikri'nin yazdığı kontrol altyapısı (`bkm.sp_Aylik*Kontrol` + `Fin_AyKapanis` + onlarca `kontrol_*` view + `mhsMizan_vw`). ✅ Üretildi: (1) `sp_KapanisMudahaleKontrol_v2` 3 SP'yi konsolide+geliştirir (CAR+FAT+MHS, tüm-dönem, özet/detay-drill, severity+forensic) — `sorgular/2026-06-22-muhasebe-kontrol-v2.sql` (+DUZ sürüm), **Fikri deploy edecek**. (2) `muhasebe-denetci` skill (10 kontrol + Benford/çapraz/iz-sürme → Türkçe rapor). **KALAN:** (a) Fikri SP deploy + v2 çıktı onayı, (b) GM dashboard "Muhasebe/Kontrol" paneli (özet→tıkla→detay drill; SP'leri Dapper EXEC), (c) bu keşfi sema'ya yaz (mhs/car/kontrol entity+bridge — ikiz yükümlülük borcu), (d) Fin_AyKapanis 2026 ayları eksik → girilmeli. İşaret: fisBA=0 alacak/gelir, fisBA=1 borç/gider (doğrulandı). GM odaklı (CFO değil).
