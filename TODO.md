...
- [~] **B-50 ⚡ MOBİL TASARIM POLISH** (asistan-ui skill) — ✅ KOD YAZILDI commit'siz: KPI mobil-kompakt (net üstte + İşlem/İade 2-grid) · KPI `text-primary-content`→`text-white` (corporate pc KOYU=mavi-üstü-siyah bug) · kart whitespace-nowrap+chevron+active:scale · hedef `_period=="ay"` guard (günlük net/aylık hedef %10,5 bug). **SABAH: build:css + dotnet build + mobil 390px teyit + COMMIT (Home/Magaza/Eticaret.razor uncommitted).** Kalan: header sıkışık · Operasyon/Envanter/Müşteri KPI mobil · dönem seçici tam-genişlik. **(YENİ 14.06)**
- [ ] **B-51 ⚡ APP DİLİ → WEB/BLAZOR UYARLAMA** (asistan-ui skill, mockup ONAYLANDI 14.06) — Kullanıcı mobil app mockup'ı çok beğendi ("işte bu"). Blazor'a responsive uyarla (mobil app + masaüstü genişlemiş). Öğeler: (1) **Gradient hero CAROUSEL** — swipe scroll-snap kart: Ciro→İşlem→Sepet Ort→İade + dot gösterge (DaisyUI primary gradient `from-primary to-primary/80`, hardcode hex YOK). (2) **İkon-kart mağaza** — renk-kodlu daire ikon (success/info/warning token) + sağda tutar + **trend oku** (↑↓ success/error). (3) Dönem segment pill. (4) E-tic ayrı gradient kart (info). (5) Alt nav korunur. **Trend için WoW verisi GEREK** → `StoreCard.Wow` + `GetPeriodAsync` geçen-dönem net sorgu (şu an YOK, Queries.cs:22 storeSql). Önce Home (mükemmel örnek) → beğenilirse diğer 5 sayfa aynı dil. **Tier 3 → plan-first.** Mockup referans: show_widget `bkm_mobil_app_hero_carousel`. **(YENİ 14.06)**
