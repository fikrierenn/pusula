...
- [~] **B-122 Kontrol paneli SP perf — date-range (sargable)** — 24.06. SP'de `JOIN @Donemler ON DonemYil=YEAR(tarih) AND DonemAy=MONTH(tarih)` non-sargable → index boşa, full scan (CAR 1413ms). `@Donemler`'e AyBas/AySon eklendi, 3 join `tarih>=AyBas AND <AySon` (CAR `IX_car_cTarih` / FAT `IX_fat_7` seek). Source düzeltildi (`sorgular/2026-06-22-muhasebe-kontrol-v2.sql`). **KALAN: (1) Fikri SP redeploy, (2) `mhsFisBaslik.fisTarih` İNDEKSSİZ → MHS branch 15M full-scan (1494ms) asıl darboğaz; DBA index önerisi: `CREATE INDEX IX_mhsFisBaslik_fisTarih ON mhs.mhsFisBaslik(fisbSirketID, fisTarih)`.** + panel ilk-yük tek-dönem zaten yapıldı (69e1d36).
