...
- [~] **B-45 ⭐ DEVAM — BKM-Asistan PWA-içi** (plan-20, Faz-1 başladı 18.06). LLM=**Gemini Flash + Groq fallback** (Anthropic değil — kullanıcı kararı). ✅ Faz-1 LLM katmanı commit (9783efd): `ILlmProvider` agnostik + GeminiProvider(REST)+GroqProvider(OpenAI-uyumlu)+FallbackLlmProvider + SaltOkumaGuard. ✅ Araçlar+loop YAZILDI ama **build DOĞRULANMADI, uncommitted**: AsistanAraclar.cs (sql_sorgu salt-okuma+PII maske / sema_oku / ornek_sql_bul golden-record / gorev_*), AsistanService.cs (tool-use loop + sema-kurallı sistem-prompt), Program.cs (DI). **YARINA:** (1) build doğrula+düzelt+commit · (2) Asistan.razor veri-sorgu modu (mevcut görev-taslak + yeni soru-cevap) · (3) Gemini-içi çok-model rotasyon (GEMINI_MODELS liste → RPD havuzu) sonra Groq · (4) canlı test "dün ciro?" + güvenlik (yazma reddi). Plan-20 master (yetenek matrisi/öğrenen katman/PII/repo-sömürü). Faz-2 mail/takvim (Gmail/GCal), Faz-3 ajanda.
