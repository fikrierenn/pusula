...
- [ ] **B-155 SinavQueries + SinavModels (plan-35)** — 19.08. `/sinav` sayfasının veri katmanı: 5 metot (sezon KPI · ödeme sağlık · eksik kalem · yan ürün attach · iade radarı). Kaynak `sorgular/2026-08-19-sinav-*.sql` (7 dosya, canlı doğrulanmış). ZORUNLU: 3-parçalı isim (`Db.OpenAsync` master bağlamı) · `OUTER APPLY TOP 1` SiparisDetay fan-out'a karşı · `@DonemId` PARAMETRE (hardcode 8 geçmişi siler — 8.998 fiş vakası) · tutar `TotalPrice − VatTotal` (KDV hariç) · `SinavUrun.Fiyat` (SiparisDetay.BirimFiyat NULL).
- [ ] **B-156 Sinav.razor 5 bölüm (plan-35)** — 19.08. DaisyUI token, hardcode hex YOK. Her tutarın yanında "KDV hariç" etiketi. Dönem dropdown. Flag çelişkisi > 0 → `text-error`. Eksik kalem tablosunda stok var/yok rozeti. Mobil 375px + `overflow-x-auto`. 300 satırı aşarsa sub-component (code-behind DEĞİL — markup RenderFragment kuralı).
- [ ] **B-157 NavRegistry + DI kaydı (plan-35)** — 19.08. `new NavItem("sinav", "Sınav", ...)` + `ServiceRegistration.cs` ilgili `AddBkm*` grubuna (Program.cs değişmez).
