...
- [ ] **B-121 Monolit "konular karışıyor" yapısal fix (23.06 kullanıcı — sabaha)** — paylaşılan dosya bleed'i (Program.cs DI / NavRegistry / app.css / TODO) farklı klasöre rağmen karışmaya sebep. 3 kademe: (1) **DI feature-extension** — her alan kendi `AddMuhasebe()`/`AddAsistan()`/`AddBkmData()` uzantısı, Program.cs tek-satır → hunk-split biter (en yüksek ROI, Tier-3 refactor). (2) `.gitattributes` `dashboard/wwwroot/app.css linguist-generated` (diff gürültüsü). (3) Kural: paylaşılan dosya edit'i kendi özellik commit'iyle gider, ayrı bucket'a düşmez. Not: bleed tamamen bitmez (monolit), disiplinle yönetilir. app.css gitignore'lanAMAZ (csproj fallback).
- [ ] **B-120 Kontrol paneli SP `SUM(bit)` fix** — `sp_KapanisMudahaleKontrol_v2` OZET'te `SUM(SonradanDeg/GecGiris/YuvarlakTutar/Mukerrer/GirenOnaylayanAyni)` bit toplayamıyor ("sum işleci için bit geçersiz") → `SUM(CAST(... AS int))`. SP `sorgular/2026-06-22-muhasebe-kontrol-v2.sql`, **Fikri deploy eder**. Panel bu yüzden çalışmıyor.
