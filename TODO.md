...
- [x] ✅ **B-108** takvim → `BkmPanel.dbo.PanelTakvim` — KAPALI 17.06. TakvimService tatil günleri (ulusal+dini+arife) DB-backed (cache JSON yerine); görev/brief/tahmin SQL ile sorgulayabilir. ApidenYenile DELETE+INSERT (okul tipleri korunur), okul dönem JSON ayrı kalır. Dapper global `DateOnlyTypeHandler` (SQL date↔DateOnly, Program.cs). Smoke: PanelTakvim oluştu, okuma temiz. ⚠️ boş başlar → /tahmin "Yenile" ile API'den dolar.
