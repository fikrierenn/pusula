...
- [~] **B-122 Kontrol paneli SP perf — sargable + mevcut index seek (YENİ INDEX GEREKMEZ)** — 24.06. (1) `@Donemler` AyBas/AySon → 3 join `tarih>=AyBas AND <AySon` (CAR `IX_car_cTarih`/FAT `IX_fat_7` seek, YEAR/MONTH non-sargable'dı). (2) **MHS: `b.fisbSirketID = yıl−2020` eklendi → mevcut `mhsFisBaslik_FisListe(fisbSirketID, fisTarih)` index SEEK (1494ms→66ms, 22x). DDL/yeni index GEREKMEZ** (kullanıcı index atamıyor — mevcut index kullanıldı). Source `sorgular/2026-06-22-muhasebe-kontrol-v2.sql`. Tahmini tek-dönem <1s. **KALAN: Fikri SP redeploy.** + ilk-yük tek-dönem (69e1d36).
