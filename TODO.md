...
- [x] ✅ **B-109** forecast portaldan tetikleme — KAPALI 17.06. `ForecastService` Process ile `python run.py` (cwd scripts/forecast, max 10dk, exit/stderr loglu). /tahmin "Tahmini Çalıştır" butonu → bitince çıktı JSON güncel → sayfa Yukle. Python KALDI (statsforecast AutoARIMA/ETS C# yok — mutabakat riski, kullanıcı kararı). Sabit komut (injection yok), auth'lu sayfa. python 3.12 + run.py doğrulandı; tam çalıştırma gerçek ortam (201 DB).
