...
- [~] **B-118 Mizan / likidite dashboard (plan-25)** — ✅ **YAZILDI 23.06** (B-117'den AYRI mali-tablo). `Mizan.razor` (route `/mizan`) + `MizanQueries.cs` + `MizanModels.cs`. Kaynak `mhs.mhsMizan_vw` (view tanımı doğrulandı: fisBA=0=Alacak/1=Borç, Bakiye=Borç−Alacak). Özet kart (nakit 100+101+102+108 / verilen-çek 103 ayrı / alıcı 120 / satıcı 320 / KDV net) + ana-hesap mizan tablo + alt-hesap drill, dönem seçici (fisSirketID=yıl−2020). Açılış/devir dahil → gerçek bakiye. **Canlı doğrulama:** denge Borç=Alacak=21,48 mlr ✓, nakit +578,6M (103 hariç düzeltildi, ilk -219M yanlıştı), verilen-çek 797,6M. Build 0 hata, route 200. **KALAN:** login-arkası görsel onay · gelir tablosu/K-Z (6xx-7xx) Faz-2 · sema mhs yazımı (ikiz borç). SQL arşiv `sorgular/2026-06-23-mizan-kesif.sql`.
