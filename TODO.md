...
- [x] ~~**V-08 Türkçe tanımlayıcı kapısı — kapsam dar**~~ — ✅ KAPANDI 19.09. `dashboard`
      kapsama alındı ama **ÇIRCIR** ile: ölçüldü, orada **2.431 bulgu / 84 dosya** var.
      İki seçenek de kötüydü — kapsama almamak dashboard'ı **kör** bırakıyordu, bloklamak
      ise kapıyı **ilk gün kapattırırdı** (kapının kendi başlığı: "kimse 500 ihlalli bir
      kapıyı açık tutmaz"). Üçüncü yol: her dosyanın bugünkü sayısı **tavan**
      (`tools/turkce-taban.json`); **artış kırar**, azalış "tabanı sıkıştır" der, **yeni
      dosya temiz olmak zorunda**. Borç dondurulmuş ve **ekranda yazılı**, gizli değil.
      Kırmızı kip iki kez: mevcut dosyaya yeni ihlal (taban 79 → 83) · yeni dosya (taban 0).

- [ ] **V-23 Dashboard Türkçe borcu — 2.431 → 2.308, ilk sıkıştırma yapıldı** (19.09)
      ⚠ ÖLÇÜM SAYIYI YENİDEN TANIMLADI: 2.431'in bir kısmı Türkçe borç DEĞİLDİ, **ak
      listede olmayan İngilizce sözcüktü** (`html · export · context · alerts · provider`).
      66 benzersiz sözcük ayrıldı: **30 İngilizce** + **16 alan terimi** ak listeye
      girdi (GMY kararı: `cari · mizan · barem · sadakat · satinalma` çevrilmez —
      `Vardiya`/`Vrd` ile aynı sınıf, alan dili), **20 genel Türkçe sözcük BORÇ olarak
      kaldı** (`detay · filtre · tablo · satir · olustur · tahmin…` — karşılığı tam).
      Taban sıkıştı: 84 → **80 dosya**, 2.431 → **2.308** (4 dosya tamamen temizlendi,
      artan dosya 0). Kalan borç GERÇEK Türkçe tanımlayıcı.
      ⚠⚠ **19.09 İKİNCİ TUR — SAYI YİNE YENİDEN TANIMLANDI, BU SEFER YUKARI: 2.308 →
      3.516** (commit 1d0c05c). Sebep borç artması DEĞİL, kapının **kör noktası**ydı ve
      iki katmanlıydı: (1) `.razor` uzantısı süpürmede yoktu — 61 UI dosyası hiç
      taranmamıştı; (2) uzantı eklendikten SONRA bile sabotaj yakalanmadı, çünkü
      `@code { … }` gövdesi atılıyordu (dashboard .razor'larında C#'ın TAMAMI orada).
      Kırmızı kip iki tur koştu: ilkinde YEŞİL kaldı → düzeltmenin yetmediği ölçüldü.
      Taranan dosya 131 → **192**. Borç GÖRÜNÜR oldu, artmadı.
      **TETİK(koşul: dashboard'da bir dosyaya dokunulduğunda o dosyanın tabanı sıkıştırılır)**
      İlk uygulama: `ExcelExport.cs` 1→0 · `ExcelButton.razor` 4→0 (commit 697a7cb).
