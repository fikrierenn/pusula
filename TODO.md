...
- [ ] **B-111** ⭐ **WMS Bekleyen Sipariş Doluluk Göstergesi** (M, dashboard-oneri 17.06) — kaç sipariş raflanmayı bekliyor + gecikme riski (real-time). Veri: `wms_bekleyen_toplama` metriği metrics.yaml'da formülüyle HAZIR (`depo.emir` + `J_ORDER_DETAILS.STATUS [2000,2002,2003,2007]`) ama dashboard'a taşınmamış. Operasyon sayfasına mini-kart. **⚠️ 18.06 KEŞİF (parked): "metrik hazır" YANILTICI — metrik J_ORDERS (e-tic) tarafı; DerinSIS `depo.emirAyr.emTamam=0` KİRLİ (2.422 emir/2M satır/4,3M adet, 2021 zombi, temiz açık/kapalı bayrağı yok). İki yol: A) emirAyr recency-filtreli (son 7g)+zombi-uyarısı (düşük efor) · B) J_ORDERS e-tic (temiz ama Tier-2/3). Kanıt+SQL: `sorgular/2026-06-18-wms-bekleyen-kesif.sql`; sema `metrics.yaml UYARI_emirAyr`.**
