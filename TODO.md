...
- [ ] **B-110** ⭐ **Tedarikçi/Yayınevi Performans Paneli** (M, dashboard-oneri 17.06) — yayınevi/marka bazında ciro + iade oranı + stok devir hızı yan yana; devir<1.5× → sipariş-kesme sinyali. Veri: `urnMrk` + `irsHrk` + `urnKtgr2` (köprü kanıtlı). CFO "bu yayınevinden sipariş kesmeli miyim?" = sermaye kararı. Yeni sayfa/kart.
- [ ] **B-111** ⭐ **WMS Bekleyen Sipariş Doluluk Göstergesi** (M, dashboard-oneri 17.06) — kaç sipariş raflanmayı bekliyor + gecikme riski (real-time). Veri: `wms_bekleyen_toplama` metriği metrics.yaml'da formülüyle HAZIR (`depo.emir` + `J_ORDER_DETAILS.STATUS [2000,2002,2003,2007]`) ama dashboard'a taşınmamış. Operasyon sayfasına mini-kart.
- [ ] **B-112** Dashboard hızlı kazanımlar (S, dashboard-oneri 17.06): (a) Ödeme grubu trendi (nakit↑=stres sinyali, `SalesPayments`+encore_odeme_grup) · (b) İade sebebi analizi (`RefundReasons.Type=0`) · (c) Kampanya kaynak kırılımı (`SalesProductCampaigns.Source` 0/1/2/3) · (d) Kasa saatleri yoğunluk (`Sales.SaleDate` saat dilimi). Hepsi mevcut sorgu varyasyonu, yeni tablo yok.
...
- [ ] **B-45 ⭐ YAPILACAK (karar 17.06)** [TIER 3 plan-first] **BKM-Asistan = PWA-içi** (Telegram DEĞİL — karar 17.06: ekstra kanal/bağımlılık gereksiz, PWA zaten var). Mevcut **dashboard PWA + `Asistan.razor`** içinde chat: CFO telefondan (PWA) "dün kargo/ciro ne oldu" → Claude API **tool-use loop** → sema/Dapper/MCP/gm-rapor araçlarıyla SQL/rapor → Türkçe cevap + kart/grafik (dashboard bileşenleri). B-84 auth + drill/sayfalarla bütünleşik (tek yer). **LLM:** Claude API tool-use (mevcut yerel `LlmService` qwen tool-use'da zayıf → Anthropic SDK; veya hibrit). Telegram.Bot bağımlılığı YOK. **Harness referansı: learn-claude-code** — tool-use loop + dispatch-map (araç=handler) + permission (s03) + subagent (s06) + bağlam-sıkıştırma (s08). Kurmadan: API maliyet + araç-allowlist (salt-okuma SQL!) + mimari plan (Tier-3). `asistan/` POC mantığı (prompt/parse) referans.
