...
- [x] ✅ **B-108** takvim TAM DB → `BkmPanel` — KAPALI 17.06. TatilGünleri `PanelTakvim` (ulusal+dini+arife, cache JSON yerine) + okul dönem `PanelOkulDonem` + sınav `PanelTakvim` Tip=Sinav. okul-takvimi.json → ilk açılış seed-if-empty (2 dönem+4 sınav doğrulandı). Görev/brief/tahmin SQL sorgulayabilir. ApidenYenile DELETE Ulusal/DiniBayram + INSERT (okul/sınav korunur). Dapper global `DateOnlyTypeHandler` (SQL date↔DateOnly). ⚠️ tatil boş başlar → /tahmin "Yenile" ile API'den dolar; okul JSON'dan seed olur.
