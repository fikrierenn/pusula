...
# -*- coding: utf-8 -*-
"""
Generate comprehensive Excel file for Sepet Büyüklüğü Etkisi analysis
"""
...
for row_num in range(2, len(raw_data) + 2):
    fill = PatternFill(start_color="F8FAFC", end_color="F8FAFC", fill_type="solid") if row_num % 2 == 0 else PatternFill()
...
ozet_headers = [
    "Ay", "3AL2ÖDE Fiş", "Diğer Fiş", "Kampanyasız Fiş", "Toplam Fiş",
    ...
    "3AL2ÖDE NetCiro", "Diğer NetCiro", "K.sız NetCiro", "Toplam NetCiro", "3AL2ÖDE Pay%"
]
...
# Format Özet data cells
for row_num in range(2, total_row + 1):
    fill = PatternFill(start_color="F8FAFC", end_color="F8FAFC", fill_type="solid") if row_num % 2 == 0 else PatternFill()

    for col_num in range(1, 26):
        cell = ws_ozet.cell(row=row_num, column=col_num)
        if row_num < total_row and fill.start_color.rgb:
            cell.fill = fill
        cell.border = THIN_BORDER

        if col_num == 1:
            cell.alignment = Alignment(horizontal="center")
        elif col_num in [2, 3, 4, 5, 17, 18, 19, 20, 21, 22, 23, 24]:
            cell.number_format = '#,##0'
            cell.alignment = Alignment(horizontal="right")
...
kpi_data = [
    ...
    ("Sepet Çarpanı (3AL2ÖDE / K.sız)", "=E4/E6"),
...
print(f"SUCCESS: Excel file created at {output_file}")
