...
def cek_maliyet(zc, veri, ciro_ay):
    ...
    zc.execute("""
        ...
        GROUP BY b.Yil, b.Ayindex, p.AltLokasyon""",
               FM_YILLIK_SINIR / 12.0, ONCEKI, CARI, son_ay)
    ay_sube = {}
    for (yil, ay_, sube, kisi_ay, mal_, brut, isvs, isvi, net_, fms, fmt, fmy, fmsn) in zc.fetchall():
        ay_sube[(int(yil), int(ay_), sube)] = {
            "kisi_ay": int(kisi_ay), "maliyet": float(mal_ or 0), "brut": float(brut or 0),
            "isveren_sgk": float(isvs or 0), "isveren_issizlik": float(isvi or 0),
    ...
    def _pencere(yil, aylar, subeler):
        ...
        d["aylar"] = list(aylar)
        if d["kisi_ay"]:
            d["kisi_ay_basi_maliyet"] = d["maliyet"] / d["kisi_ay"]
            d["kisi_ay_basi_ciro"] = d["ciro_kdvharic"] / d["kisi_ay"]
            d["fm_kisi_ay_basi_saat"] = d["fm_saat"] / d["kisi_ay"]
            d["fm_yillik_kisi_basi_saat"] = d["fm_saat"] / d["kisi_ay"] * 12.0
        if d["ciro_kdvharic"]:
    ...
    veri["fazla_mesai"] = {
        ...
        "kumulatif": {"fm_saat25": k25["fm_saat"], "fm_saat26": k26["fm_saat"],
                      ...
                      "kisi_basi_yillik25": k25.get("fm_yillik_kisi_basi_saat", 0.0),
                      "kisi_basi_yillik26": k26.get("fm_yillik_kisi_basi_saat", 0.0)},
        ...
        "fiili": {"fm_saat26": p26["fm_saat"], "fm_saat25": p25["fm_saat"],
                  ...
                  "kisi_basi_yillik26": p26.get("fm_yillik_kisi_basi_saat", 0.0),
                  "kisi_basi_yillik25": p25.get("fm_yillik_kisi_basi_saat", 0.0),
        ...
        "varsayim": ("SEZON penceresinde (%s) ölçülür. İşgücü ihtiyacı kişi sayısıyla doğru "
                     "orantılı kabul edilir; %d kişi-aylık "
                     ...
                     % (maliyet["sezon"]["etiket"], eksik_kisi_ay, AY_NORMAL_SAAT,
                        int(FM_YILLIK_SINIR))),
