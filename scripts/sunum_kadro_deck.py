...
# ================================================================= 1 KAPAK
s = add("Başlık Slaydı")
setph(s, 0, "Kadro mu büyüdü, iş mi büyüdü?")
setph(s, 1, "Sezon 2026 · beş mağaza kadrosu · üç POS mağazası iş hacmi · 2025 ile aynı takvim dönemi")

# ================================================================= 2 GUNDEM
s = add("Yalnızca Başlık"); setph(s, 0, "Beş soruda cevap")
ag = [("users", "Kadro ne oldu?", "Taban 30.06 · sezon içi hareket · sezonluk."),
      ("package", "İş ne oldu?", "Elleçlenen ürün adedi ve ciro, aynı dönemde."),
      ("zap", "Kişi başına düşen iş", "Asıl ölçü: adam başı kaç ürün, kaç lira."),
      ("layers", "Artış nereye gitti?", "Yönetime mi, rafın önüne mi?"),
      ("shield-check", "Gelebilecek itirazlar", "Enflasyon · kurumsal · kasa değişimi · takvim."),
      ("alert-triangle", "Düzeltmemiz gereken", "Yeni alınanın ilk iki hafta tutunması.")]
...
# ================================================================= 3 TEK SAYFA OZET
s = add("Yalnızca Başlık"); setph(s, 0, "Tek sayfada cevap")
...
tb(s, 0.9, 3.6, 11.5, 1.0,
   [("Kadro farkı sezon başlamadan önce kurulmuştu; sezon içinde kadro küçülürken iş hacmi %s büyüdü."
     % yzd(d_adet).replace("+", ""), 15, True, DRED)], anchor=MSO_ANCHOR.MIDDLE)

card(s, 0.6, 4.8, 5.9, 1.15, MGREY)
tb(s, 0.85, 4.92, 5.4, 0.3, [("KADRO", 10, True, GREY)])
...
tb(s, 7.0, 5.22, 5.4, 0.65,
   [("iş, kadronun %s katı hızla büyüdü" % ("%.1f" % kat).replace(".", ","), 15, True, RED)])
sig(s)

# ================================================================= 4 DIVIDER
s = add("Bölüm Üst Bilgisi"); setph(s, 0, "Kadro ne oldu?")
setph(s, 1, "Bölüm 1 — taban 30 Haziran, kesim 31 Ağustos"); sig(s)

# ================================================================= 5 KADRO AKISI
s = add("Yalnızca Başlık"); setph(s, 0, "Fark sezon başlamadan önce oluştu")
akis = [("2025 tabanı", "%d" % k5["kadrolu_taban25"], "30 Haziran 2025", MGREY),
        ("1 Tem'den önce eklenen", "%+d" % taban_fark, "sezon başlamadan", RED),
...
tb(s, 7.25, 3.92, 5.2, 1.55,
   [("Sezon içinde kadro büyümedi.", 15, True, DRED),
    ("Kadrolu %d → %d (%+d). Geçen yıl da aynı yönde (%+d): sezonda alınan kadrolu, ayrılanların "
     "yerine geldi — kadroyu büyütmedi." % (k5["kadrolu_taban26"], k5["kadrolu_kesim26"],
                                            sezon_ici_26, sezon_ici_25), 12, False, INK)], sp=1.15)
sig(s)

# ================================================================= 6 BES MAGAZA TABLOSU
s = add("Yalnızca Başlık"); setph(s, 0, "Mağaza mağaza kadro")
basliklar = ["Mağaza", "Kadrolu 30.06 · 2025", "Kadrolu 30.06 · 2026", "Kadrolu 31.08 · 2026",
             "Sezon içi hareket", "Sezonluk 31.08"]
satirlar = [basliklar]
for m in v["magaza_kadro"]:
    satirlar.append([tr_title(m["sube"]), str(m["kadrolu_taban25"]), str(m["kadrolu_taban26"]),
                     str(m["kadrolu_kesim26"]),
                     "%+d" % (m["kadrolu_kesim26"] - m["kadrolu_taban26"]),
                     str(m["sezonluk_kesim26"])])
satirlar.append(["TOPLAM", str(k5["kadrolu_taban25"]), str(k5["kadrolu_taban26"]),
                 str(k5["kadrolu_kesim26"]), "%+d" % sezon_ici_26, str(k5["sezonluk_kesim26"])])
t = s.shapes.add_table(len(satirlar), 6, Inches(0.6), Inches(1.55), Inches(12.05), Inches(3.5)).table
for w, gen in zip(range(6), (2.6, 2.0, 2.0, 2.0, 1.85, 1.6)):
    t.columns[w].width = Inches(gen)
for r, row in enumerate(satirlar):
    for c, val in enumerate(row):
        cell = t.cell(r, c); cell.text = val
        son = (r == len(satirlar) - 1)
        for para in cell.text_frame.paragraphs:
            para.alignment = PP_ALIGN.LEFT if c == 0 else PP_ALIGN.CENTER
            for run in para.runs:
                run.font.size = Pt(11.5); run.font.name = "Calibri"
                run.font.bold = (r == 0 or son)
                run.font.color.rgb = WHITE if r == 0 else INK
        cell.fill.solid()
        cell.fill.fore_color.rgb = RED if r == 0 else (LGREY if son else (WHITE if r % 2 else RGBColor(0xFA, 0xFA, 0xFA)))
tb(s, 0.6, 5.25, 12.05, 0.7,
   [("Heykel ve Şura POS raporlamasında yok — iş hacmi üç mağazada ölçülür, kadro hareketi beş mağazanın "
     "tamamında verilir.", 11, False, GREY)])
sig(s)

# ================================================================= 7 DIVIDER
s = add("Bölüm Üst Bilgisi"); setph(s, 0, "İş ne oldu?")
setph(s, 1, "Bölüm 2 — okul açılışına hizalı %d gün, Sınav hariç" % gun); sig(s)

# ================================================================= 8 IS HACMI
s = add("Yalnızca Başlık"); setph(s, 0, "Aynı dönemde iş hacmi")
...
kpi(s, 8.75, 1.5, 3.9, "KADRO", yzd(d_kadro), "%d → %d kişi" % (kadro25, kadro26), CHAR, 32)
...
tb(s, 7.25, 3.85, 5.2, 1.75,
   [("Ürün adedi neden birincil ölçü?", 14, True, DRED),
    ("Adet enflasyondan etkilenmez — fiilen kasadan geçen, rafa dizilen, depodan çıkan mal miktarıdır. "
     "Ciro fiyat artışıyla şişebilir; adet şişmez.", 12, False, INK)], sp=1.15)
...
# ================================================================= 9 KISI BASI MAGAZA
s = add("Yalnızca Başlık"); setph(s, 0, "Kişi başına düşen iş")
kats, s1, s2 = [], [], []
...
# ================================================================= 10 4 YILLIK TREND
s = add("Yalnızca Başlık"); setph(s, 0, "Kişi başına iş — dört yıllık seyir")
...
# ================================================================= 11 DIVIDER
s = add("Bölüm Üst Bilgisi"); setph(s, 0, "Artış nereye gitti?")
setph(s, 1, "Bölüm 3 — bölüm bölüm kadro"); sig(s)

# ================================================================= 12 BOLUM KIRILIMI
s = add("Yalnızca Başlık"); setph(s, 0, "Yönetimde değil, rafın önünde")
ilk5 = buyuyen[:5]
...
tb(s, 8.35, 1.72, 4.05, 0.35, [("BÜYÜMEYEN BÖLÜMLER", 10.5, True, GREY)])
tb(s, 8.35, 2.08, 4.05, 1.2,
   [("Yönetim +0  ·  Mal Kabul +0  ·  İdari İşler +0", 13, True, CHAR),
    ("Bu üç bölüm bir kişi bile büyümedi.", 10.5, False, GREY)], sp=1.15)
card(s, 8.1, 3.5, 4.55, 1.7, RED)
tb(s, 8.35, 3.62, 4.05, 0.35, [("BÜYÜYEN BÖLÜMLER", 10.5, True, GREY)])
...
tb(s, 0.9, 5.4, 11.5, 0.85,
   [("Kadrolu %+d kişinin tamamı satış ve kasa tarafında. Yönetim kadrosu bir kişi bile büyümedi."
     % taban_fark, 13.5, True, DRED)], anchor=MSO_ANCHOR.MIDDLE)
sig(s)

# ================================================================= 12b MAGAZA x BOLUM MATRISI
s = add("Yalnızca Başlık"); setph(s, 0, "Mağaza × bölüm")
...
tb(s, 0.6, 1.5 + min(4.3, 0.32 * len(satir)) + 0.12, 12.05, 0.85,
   [("'—' o mağazada o bölüm hiç yok. Yönetim (Mağaza), Mal Kabul ve İdari İşler satırları her mağazada "
     "sıfır veya eksi: artış yalnız satış ve kasa reyonlarında.", 10.5, False, GREY)], sp=1.15)
sig(s)

# ================================================================= 12c MAGAZA PERFORMANS
s = add("Yalnızca Başlık"); setph(s, 0, "Mağaza performansı")
...
tb(s, 8.45, 3.95, 4.0, 1.7,
   [("Her mağazada aynı desen", 13.5, True, DRED),
    ("Kırmızı ve koyu çubuk (iş) gri çubuğun (kadro) hep üstünde. En çok kadro büyüyen İst. Yolu'nda "
     "bile iş, kadronun 2,2 katı hızla büyüdü.", 11.5, False, INK)], sp=1.15)
sig(s)

# ================================================================= 12d KATEGORI x BOLUM ESLESME
s = add("Yalnızca Başlık"); setph(s, 0, "Kadro, işin büyüdüğü rafa gitti")
...
tb(s, 8.45, 4.5, 4.0, 1.5,
   [("Alım rastgele değil", 13.5, True, DRED),
    ("Kadro, adedi en hızlı büyüyen raflara verildi. En yavaş büyüyen kategoride (Çocuk Kitabı) "
     "kadro azaltıldı.", 11.5, False, INK)], sp=1.15)
sig(s)

# ================================================================= 13 ITIRAZLAR
s = add("Yalnızca Başlık"); setph(s, 0, "Gelebilecek itirazlar ve cevapları")
itiraz = [
    ("\"Büyüme kurumsaldan geldi\"",
...
# ================================================================= 14 DUZELTILECEK
s = add("Yalnızca Başlık"); setph(s, 0, "Düzeltmemiz gereken taraf")
card(s, 0.6, 1.5, 5.9, 2.5, DRED)
tb(s, 0.85, 1.65, 5.4, 0.4, [("KADROLU ALIMDA TUTUNMA DÜŞTÜ", 11, True, GREY)])
tb(s, 0.85, 2.05, 5.4, 0.9, [("%95,7 → %75,0", 32, True, DRED)])
tb(s, 0.85, 2.95, 5.4, 0.95,
   [("İlk 14 günü tamamlama oranı. 32 alımdan 10'u kesime kadar ayrıldı (geçen yıl 24 alımdan 2). "
     "Aynı pozisyonu iki kez doldurma maliyeti.", 11, False, INK)], sp=1.1)
card(s, 6.75, 1.5, 5.9, 2.5, MGREY)
tb(s, 7.0, 1.65, 5.4, 0.4, [("SEZONLUK TARAFTA TERSİ", 11, True, GREY)])
tb(s, 7.0, 2.05, 5.4, 0.9, [("%93,9 → %95,1", 32, True, YESIL)])
tb(s, 7.0, 2.95, 5.4, 0.95,
   [("Sezon kadrosunun tutunması İYİLEŞTİ. Yani sorun sezonluk alımda değil, kadrolu alımın ilk "
     "haftasında.", 11, False, INK)], sp=1.1)

rrect(s, 0.6, 4.2, 12.05, 1.75, LGREY, RED, lw=1.5)
tb(s, 0.9, 4.35, 11.5, 1.5,
   [("Aksiyon", 13.5, True, DRED),
...
# ================================================================= 15 KAPANIS
s = add("Başlık Slaydı")
setph(s, 0, "Kadro değil, iş büyüdü")
setph(s, 1, "Taban farkı sezon başlamadan kurulmuştu · sezon içinde kadrolu %+d · ürün adedi %s · "
            "kişi başı iş %s" % (sezon_ici_26, yzd(d_adet), yzd(d_kb)))
