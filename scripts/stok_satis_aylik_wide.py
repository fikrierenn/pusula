...
maliyet = {}   # stkID -> (birim_maliyet, tarih)
cur.execute("""
    WITH la AS (
...
# Başlık — maliyet kolonları eklendi; ay blokları sona
HEAD = ["stkID", "BarkodAna", "stkAd", "mrkAd", "Kategori3",
        "Üst Fiyat", "Son Alış Maliyeti", "Son Alış Tarihi", "Maliyet Kaynak",
        "Toplam Etiket Değeri", "Toplam Maliyet Değeri",
        "depo_stok", "FSM_stok", "OZL_stok", "IST_stok", "Toplam Stok", "Son 1 yıl Satış", "Stok Dönüş Hızı"]
for ch in CHANNELS:
...
UF, SM, ET, MD = COL["Üst Fiyat"], COL["Son Alış Maliyeti"], COL["Toplam Etiket Değeri"], COL["Toplam Maliyet Değeri"]
...
for r, sid in enumerate(order, start=2):
    ...
    d = mstok.get(sid, {})
    mb, mt, mk = maliyet.get(sid, (0, None, "Yok"))
    row = [sid, brk, ad, mrk, kat,
           fy,                                          # Üst Fiyat
           mb,                                          # Son Alış Maliyeti (fatura/devir)
           mt,                                          # Son Alış Tarihi
           mk,                                          # Maliyet Kaynak (Fatura/Devir/Yok)
           f"={UF}{r}*{TS}{r}",                         # Toplam Etiket Değeri = ÜstFiyat×ToplamStok
           f"={SM}{r}*{TS}{r}",                         # Toplam Maliyet Değeri = SonAlış×ToplamStok
           depo.get(sid, 0),                            # depo
...
out = os.path.join(os.path.dirname(__file__), "..", "raporlar",
                   f"stok-satis-aylik-wide-{today.strftime('%Y%m%d')}.xlsx")
