...
if __name__ == "__main__":
    ...
    for row_idx, row_data in enumerate(veri_rows, 2):
        for col_idx, value in enumerate(row_data, 1):
            ...
            if col_idx in [3, 4, 5, 6, 7, 9, 10, 11]:
    ...
    for row_idx, ay in enumerate(aylar, 3):
        ws_ozet.cell(row=row_idx, column=1, value=ay).font = Font(bold=True)

        col_idx = 2
        for grup in gruplar:
            row_data = veri_dict.get((ay, grup))
            if row_data:
                ws_ozet.cell(row=row_idx, column=col_idx, value=row_data[3]).number_format = '#,##0.00'
                ws_ozet.cell(row=row_idx, column=col_idx+1, value=row_data[5]).number_format = '#,##0.00'
                ws_ozet.cell(row=row_idx, column=col_idx+2, value=row_data[10]).number_format = '#,##0.00'
                ws_ozet.cell(row=row_idx, column=col_idx+3, value=row_data[2]).number_format = '#,##0'
            else:
                for i in range(4):
                    cell = ws_ozet.cell(row=row_idx, column=col_idx+i, value=0)
                    if i != 3:
                        cell.number_format = '#,##0.00'
                    else:
    ...
    ws_ozet.sheet_properties.tabColor = "2196F3"
    ...
    # KPI 1
    ws_dash.cell(row=1, column=1, value="TOPLAM FİŞ SAYISI").font = kpi_style_title
    ws_dash.cell(row=1, column=1).fill = kpi_fill
    ws_dash.cell(row=2, column=1, value="=SUM(Veri!C2:C31)").font = kpi_style_value
    ws_dash.cell(row=2, column=1).fill = kpi_fill
    ws_dash.cell(row=2, column=1).number_format = '#,##0'

    # KPI 2
    ws_dash.cell(row=1, column=2, value="TOPLAM BRÜT CİRO").font = kpi_style_title
    ws_dash.cell(row=1, column=2).fill = kpi_fill
    ws_dash.cell(row=2, column=2, value="=SUM(Veri!I2:I31)").font = kpi_style_value
    ws_dash.cell(row=2, column=2).fill = kpi_fill
    ws_dash.cell(row=2, column=2).number_format = '#,##0.00'

    # KPI 3
    ws_dash.cell(row=1, column=3, value="TOPLAM NET CİRO").font = kpi_style_title
    ws_dash.cell(row=1, column=3).fill = kpi_fill
    ws_dash.cell(row=2, column=3, value="=SUM(Veri!K2:K31)").font = kpi_style_value
    ws_dash.cell(row=2, column=3).fill = kpi_fill
    ws_dash.cell(row=2, column=3).number_format = '#,##0.00'

    # KPI 4
    ws_dash.cell(row=1, column=4, value="GENEL İNDİRİM ORANI").font = kpi_style_title
    ws_dash.cell(row=1, column=4).fill = kpi_fill
    ws_dash.cell(row=2, column=4, value="=SUM(Veri!J2:J31)/SUM(Veri!I2:I31)*100").font = kpi_style_value
    ws_dash.cell(row=2, column=4).fill = kpi_fill
    ws_dash.cell(row=2, column=4).number_format = '0.0'

    # KPI 5
    ws_dash.cell(row=3, column=1, value="3AL2ÖDE ORT. SEPET").font = kpi_style_title
    ws_dash.cell(row=3, column=1).fill = kpi_fill
    ws_dash.cell(row=4, column=1, value="=AVERAGEIF(Veri!B:B,\"3Al2Ode\",Veri!F:F)").font = kpi_style_value
    ws_dash.cell(row=4, column=1).fill = kpi_fill
    ws_dash.cell(row=4, column=1).number_format = '#,##0.00'

    # KPI 6
    ws_dash.cell(row=3, column=2, value="KAMPANYASIZ ORT. SEPET").font = kpi_style_title
    ws_dash.cell(row=3, column=2).fill = kpi_fill
    ws_dash.cell(row=4, column=2, value="=AVERAGEIF(Veri!B:B,\"Kampanyasiz\",Veri!F:F)").font = kpi_style_value
...
ORDER BY Ay, Grup"""
    ...
    for path in paths_to_try:
        try:
            ...
            print(f"Path deneniyor: {path} - Hata: {e}")
            continue

    if not output_path:
        output_path = paths_to_try[0]
